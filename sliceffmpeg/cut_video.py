...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Cortar un video según timestamps en un archivo slice.txt."
    )
    parser.add_argument(
        "-i", required=True, help="Ruta completa del archivo de video de entrada."
    )
    parser.add_argument(
        "-slice", required=True, help="Ruta completa del archivo slice.txt."
    )
    ...
    try:
        ...
        logging.error(
            f"El archivo slice.txt no se encontró en la ruta especificada: {slice_file}"
        )
        return
    ...
    for i, line in enumerate(lines):
        try:
            ...
            logging.info(
                f"Transformando tiempos: {start_time} -> {time_init} segundos, {end_time} -> {time_final} segundos"
            )

            command = generate_ffmpeg_command(
                input_video, time_init, time_final, output_video
            )
